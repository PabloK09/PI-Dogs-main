...
export default function FilterSort({setCurrentPage, setOrden}) {
  ...
  //       ...prevData
  //     };
  //     state.temperamentoTotal = state.temperamentoTotal.filter((temp)=> (temp !== e))
  //     return state
  //   })
    
  //   filtro2 = breedsState.filter((temp) =>
  //   temp.temperament?.includes(listaTemp.temperamentoTotal))
  //   console.log(filtro2)
  //   if(filtro2.length){
  ...
  return (
    <div>
      <button name="AZ" value="AZ" onClick={handleSortName}> AZ </button>
      <button name= "ZA" value="ZA" onClick={handleSortName}> ZA </button>
      <button  onClick={()=> setCurrentPage(2)}> SET PAGE </button>
          ...
          ? listaTemp.temperamentoTotal?.map((temp) => (
              <ul>
                <li>
                 ...
                 id={temp}
                 value={temp}
                //  onClick={() => handleOnClose(document.getElementById(`${temp}`).value)}
                 >X</button> 
                </li>
              </ul>
            ))
      ...
      <button name= "WL" value="WL" onClick={handleSortWeight}> WL </button>
      <button name= "WH" value="WH"> WH </button>
    </div>
  );
}
